...
/// Built-in directory names that are skipped during scan unless the
/// user explicitly opts back in. Anything generated by tooling or that
/// pollutes the index with thousands of irrelevant tiny files belongs
/// here. User can extend via the `excluded_dirs` setting.
pub const DEFAULT_EXCLUDE_DIRS: &[&str] = &[
    // VCS
    ".git", ".svn", ".hg",
    // OS / Finder cruft
    ".DS_Store", "Thumbs.db", "$RECYCLE.BIN", ".Trash", ".Trashes",
    // Package manager / build outputs
    "node_modules", "bower_components", "vendor",
    "target", "build", "dist", "out", ".next", ".nuxt", ".turbo", ".cache",
    "__pycache__", ".pytest_cache", ".mypy_cache", ".tox",
    "venv", ".venv", "env", ".env",
    // IDE / editor
    ".idea", ".vscode", ".vs",
    // misc
    ".docusaurus", ".serverless", "coverage",
];

/// Read user's custom excluded directories from settings (comma-separated).
/// Returns the merged set of defaults + user list.
fn load_excluded_dirs(state: &AppState) -> std::collections::HashSet<String> {
    let mut set: std::collections::HashSet<String> = DEFAULT_EXCLUDE_DIRS
        .iter()
        .map(|s| s.to_string())
        .collect();
    if let Ok(db) = state.db.lock() {
        if let Ok(s) = db.query_row(
            "SELECT value FROM settings WHERE key = 'excluded_dirs'",
            [],
            |r| r.get::<_, String>(0),
        ) {
...
pub const SUPPORTED_EXTS: &[&str] = &[
    // 文档
    "pdf", "doc", "docx", "ppt", "pptx", "rtf", "epub",
    // 表格
    "xls", "xlsx", "csv",
    // 文本 / 标记
    "txt", "md", "rst",
...
pub fn scan_and_index_with(
    ...
    let excluded = load_excluded_dirs(state);
    let files = collect_files_with(folder, &enabled_exts, &excluded);
    let total = files.len();
    ...
    for (i, path) in files.iter().enumerate() {
        ...
        let is_heavy = matches!(
            ext.as_str(),
            "pdf" | "jpg" | "jpeg" | "png" | "bmp" | "tiff" | "tif" | "webp" | "heic" | "heif"
        );
...
pub fn collect_files(root: &Path, enabled_exts: &[String]) -> Vec<PathBuf> {
    collect_files_with(root, enabled_exts, &default_excludes_set())
}

/// Same as `collect_files` but takes an explicit excluded-dir set. Used
/// by the indexer with the user's settings merged in.
        ...
        for entry in entries.flatten() {
            ...
            if path.is_dir() {
                let name = path
                    ...
                    .to_string_lossy()
                    .into_owned();
                // Skip dotted hidden dirs by default (most are caches /
                // OS metadata). The user can re-enable a specific dotted
                // dir by removing it from `excluded_dirs` and adding a
                // fake "include" pattern — but in practice nobody wants
                // their .Library indexed.
                if name.starts_with('.') && !excluded.contains(&name) {
                    // hidden but not in our explicit list — still skip
                    // EXCEPT for the watched root itself (we never reach
                    // here for the root because we only descend).
                    continue;
                }
                if excluded.contains(&name) {
                    continue;
                }
                result.extend(collect_files_with(&path, enabled_exts, excluded));
...
fn default_excludes_set() -> std::collections::HashSet<String> {
