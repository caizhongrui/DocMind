// macOS：使用 ort (ONNX Runtime) + tokenizers 进行语义 embedding
// Windows/Linux：存根实现，语义搜索不可用，全文搜索正常工作
...
/// Windows / Linux 存根：语义搜索不可用，全文搜索（tantivy）正常工作
/// 用户无需下载 ONNX 模型，搜索功能通过 BM25 全文索引提供
...
impl Embedder {
    pub fn load(_model_dir: &std::path::Path) -> anyhow::Result<Self> {
        anyhow::bail!("语义搜索仅支持 macOS，Windows/Linux 请使用全文搜索模式")
    }

    pub fn is_available(_model_dir: &std::path::Path) -> bool {
        false
    }

    pub fn embed(&mut self, _text: &str) -> anyhow::Result<Vec<f32>> {
        anyhow::bail!("语义搜索仅支持 macOS")
    }
}
