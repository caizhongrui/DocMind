// macOS：使用 llama-cpp-2 进行本地推理（Metal GPU 加速）
// Windows/Linux：存根实现，本地 LLM 不可用，用户应使用 API 模式
...
/// Windows / Linux 存根：本地 LLM 不可用，所有方法返回错误
/// 用户应在设置中配置 API 模式（OpenAI 兼容接口）
...
impl Llm {
    pub fn load(_model_path: &std::path::Path) -> anyhow::Result<Self> {
        anyhow::bail!("本地 LLM 仅支持 macOS，Windows/Linux 请在设置中使用 API 模式")
    }
    ...
    pub fn generate(&self, _prompt: &str, _max_new_tokens: usize) -> anyhow::Result<String> {
        anyhow::bail!("本地 LLM 仅支持 macOS，Windows/Linux 请在设置中使用 API 模式")
    }
}
